...
# shared libs it requires and a synthetic _docs/ tree, then runs the real
...
class ValidateUrlsTest < Minitest::Test
  ...
  LIBS   = Dir[File.expand_path("lib/{front_matter,reserved_slugs}.rb", __dir__)]
  ...
  def test_app_folder_without_index_is_rejected
    assert_rejected(
      { "orphan-app/privacy.md" => DOC_OK },
      "no index.md",
    )
  end

  def test_app_folder_with_only_index_still_passes
    assert_ok("solo-app/index.md" => INDEX_OK.sub("permalink: /good-app/", "permalink: /solo-app/"))
  end
end
